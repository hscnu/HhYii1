<div class="box">
    <div class="box-title c">
        ...
        <?php } ?>
        <?php if(($order_model->take_type=='配送'&&$order_model->deliver_name!=NULL)&&$condition==2){?>
            <a class="btn btn-green" href="javascript:;" onclick="logistics_examine(1)">审核通过</a>
        <?php } ?>
        <?php if($order_model->take_type=='自取'&&$condition==2){?>
        ...
        <div class="box-detail-bd">
            <div style="display:block;" class="box-detail-tab-item">
                <table>
                    ...
                    <tr>
                        <td colspan="1">标题：</td>
                        <td colspan="5"><?php echo $order_model->title; ?></td>
                    </tr>
                    <tr>
                        <td>公司/单位：</td>
                    ...
                    <tr>
                        ...
                        <td>收货地点：</td>
                        <td colspan="3"><?php echo $order_model->order_destination; ?></td>
                    </tr>
                    <tr>
                        <td colspan="1">备注：</td>
                        <td colspan="5"><?php echo $order_model->order_remark; ?></td>
                    </tr>
                </table>
            ...
            <div>
                <ul style="display: inline-block;width:30%;text-align: left;">
                    ...
                    <?php if($order_model->take_type=='配送'&&$condition==2){?>
                        <button class="btn btn-blue" type="button" onclick="chooseShr(<?php echo $order_model->id;?>);">选择送冰员</button>
                    <?php } ?>
                </ul>
...
<script>
    function fishery_examine(){
        url = '<?php echo $this->createUrl("ChangeState",array('id'=>$order_model->id,'Now_state'=>'渔业审核通过'));?>'
        $.ajax(
            {
                url:url,
                type:'post',
    ...
    function fishery_examine_back(){
        url = '<?php echo $this->createUrl("ChangeState",array('id'=>$order_model->id,'Now_state'=>'渔业退回订单'));?>'
        $.ajax(
            {
                url:url,
                type:'post',
    ...
    function logistics_examine(condition){
        if(condition==1){
            url = '<?php echo $this->createUrl("ChangeState",array('id'=>$order_model->id,'Now_state'=>'物流配送审核通过'));?>'
        }
        if(condition==2){
            url = '<?php echo $this->createUrl("ChangeState",array('id'=>$order_model->id,'Now_state'=>'物流自取审核通过'));?>'
        }
        $.ajax(
            {
                url:url,
    ...
    function logistics_examine_back(){
        url = '<?php echo $this->createUrl("ChangeState",array('id'=>$order_model->id,'Now_state'=>'物流退回订单'));?>'
        $.ajax(
            {
                url:url,
                type:'post',
