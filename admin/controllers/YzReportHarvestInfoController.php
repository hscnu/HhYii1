...
class YzReportHarvestInfoController extends BaseController {
...
//        $modelName='YzReportHarvestProduct';
...
//        }else {
//            $this->Save_detail($model, $_POST['YzReportHarvestProduct']);
//        }
//    }
//
...
//        put_msg($id);
        if (!Yii::app()->request->isPostRequest) {
            if ($isAdd == 1) {
                $data = YzReportHarvestInfo::model()->find("state='待审核' order by id");
                if (!empty($data)) {
...
//        put_msg($post);
        ...
        {
            $next_id='';
            $array=YzReportHarvestInfo::model()->findAll("state='待审核'  order by id");
