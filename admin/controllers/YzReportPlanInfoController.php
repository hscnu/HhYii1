...
class YzReportPlanInfoController extends BaseController {
    ...
    public function actionDelete($id) {
        ...
        YzReportPlanProduct::model()->deleteAll('report_order='.$data['report_order']);
        parent::_clear($id);
    }
    ...
    public function actionUpdate($id='0') {
        ...
        if (!Yii::app()->request->isPostRequest) {
            put_msg($id);
            $modelName2 = 'YzReportPlanProduct';
    ...
    function saveData2($model, $post1) {
        ...
        $check = YzReportPlanProduct::model()->find('report_order='.$report_order);
        ...
        if(empty($check)){
            ...
            $tmp = YzReportPlanProduct::model()->findAll('report_order='.$report_order.' order by product_id');
            foreach ($tmp as $k =>$v){
...
//        }else {
//            $this->Save_detail($model, $_POST['YzReportPlanProduct']);
//        }
//    }
//
...
//    }
    ...
    public function actionIndex($keywords = '',$start_date_operate='',$end_date_operate='',$start_date_report='', $end_date_report='') {
        ...
        foreach ($data as $v) {
            YzReportPlanProduct::model()->deleteAll('report_order='.$v['report_order']);
        }
...
//        put_msg($id);
        if (!Yii::app()->request->isPostRequest) {
            if ($isAdd == 1) {
                $data = YzReportPlanInfo::model()->find("state='待审核' order by id");
                if (!empty($data)) {
                    ...
                    $detailList = YzReportPlanProduct::model()->findAll('report_order=' . $model->report_order);
                    $data = array();
                    $data['model'] = $model;
...
//        put_msg($post);
        ...
        $model = YzReportPlanInfo::model()->find('report_order='.$post['report_order']);
        $model->attributes=$post;
        ...
        {
            $next_id='';
            $array=YzReportPlanInfo::model()->findAll("state='待审核'  order by id");
