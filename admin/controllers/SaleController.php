...
class SaleController extends BaseController {
    ...
    function saveData($model, $post) {
        ...
        if ($_POST['submitType'] == 'baocun'){
            $s2=str_replace('sale/index_register','sale/index_register',$url);
        } elseif($_POST['submitType'] == 'baonext'){
            $s2=str_replace('sale/index_register','sale/create',$url);
        }
    ...
    public function addcondition($model,$views,$keywords,$w){
        ...
        $is_admit=DecodeAsk('is_admit','');
        $criteria -> condition = get_like( '1' ,'id,saler_username,start_time,end_time,start_price,goods_name,goods_id,goods_weight,title',$keywords);
        //附加条件,这里的p_condtion借用了原菜单
        $p_condition=$model::model()->getInfoFromMenu($views,'p_condition');
        if(!empty($p_condition))
            $criteria->addCondition($p_condition);
        if($is_admit!=''){
            $criteria->addCondition('is_admit='.$is_admit);
        }
        if(!empty($w)){
    ...
    public function putOutAll(){
        Sale::model()->updateAll(array('is_admit' => '1'), "is_admit = '0'");
    }

    //单个审批
    public function actionPutKd($id,$keywords = '') {
        $tmp=Sale::model()->find('id = '.$id);
        $tmp->is_admit='1';
        $tmp->save();
        $this->actionIndex('拍卖审批',$keywords);
    }

    //批量审批
    public function actionPutOut($id,$keywords = '') {
        $tmp=Sale::model()->findAll('id in (' . $id . ')');
        foreach ($tmp as $v){
            $v->is_admit='1';
            $v->save();
        }
    ...
    public function actionIndex_Register($keywords = '') {
        $this->actionIndex('拍卖录入',$keywords);
    }
    ...
    */
    ...
    public function actionIndex_flow($keywords = '') {
        $w=" (DATE_FORMAT(start_time,'%Y-%m-%d')) <= '".Date('Y-m-d')."'";
        $w.=" and (DATE_FORMAT(end_time,'%Y-%m-%d')) >= '".Date('Y-m-d')."'";
        $this->actionIndex('今日热拍',$keywords,$w);
    }
