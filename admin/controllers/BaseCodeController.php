...
class BaseCodeController extends BaseController {
    ...
    public function actionIndex($keywords = '', $fType = '',$oper='') {
        ...
        $criteria -> condition = get_like('1','f_code,f_name,f_group,f_type,f_type_CN,f_order',$keywords);
        if ($fType != null){
...
;        $data = array();
    ...
    public function actionIndexVerify($keywords = '',$department_keywords='',$state_name_keywords='')
    {
        ...
        $criteria = new CDbCriteria;
        $w = '已完成';
        $criteria -> condition = get_like('1','submit_state',$w);
        $criteria -> condition = get_like($criteria -> condition,'teacher_code,teacher_name',$keywords);
