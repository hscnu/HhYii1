...
class BaseController extends CController {
    ...
    protected function _list($model = null, $criteria = null, $template = 'index', $data = array(), $pageSize = 15) {
        ...
        $criteria->condition=str_replace('AND ()','',$criteria->condition);
    ...
    {
        if($action->id=='updatedetail')return true;
    ...
    protected function  bcheckAccess($access) {
        if (!$this->check($access)) {
            if (Yii::app()->request->isAjaxRequest) {
                ajax_status(0, '没有操作权限，请重新登录！！');
            } else {
                $this->layout = false;
                $this->render('//public/error', array('message' => '对不起，没有操作权限，请重新登录！！'));
                exit;
            }
        }
    }
    ...
    protected function checkAccess($access) {
        return true;
        if (!$this->check(Yii::app()->controller->id,$access->id)) {
            if (Yii::app()->request->isAjaxRequest) {
                ajax_status(0, '对不起，没有操作权限，请重新登录！！');
            } else {
                $this->layout = false;
                $this->render('//public/error', array('message' => '无'.Yii::app()->session[$access->id].'操作权限，请重新登录！！'));
                exit;
            }
        }
        return true;
    ...
    function check($mname='pid',$oname='aid') {
        $powen=Yii::app()->session['power'];
        if(($mname=='baseInfo')&&(Yii::app()->session['F_ROLENAME']=='单位管理员')){
            return true;
        }
        return isset($powen[$mname][$oname])|| isset($powen[$mname])||($mname=='index') ? true : false;
    }


    protected function bcheck($access) {
        if (Yii::app()->session['admin_system']) {
            return true;
        }
        if (in_array($access, Yii::app()->session['role_access'])) {
            return true;
        }
        if (Yii::app()->session['TCOD'])
        {
            return true;
        }
        return false;
    ...
    public function actionupdown($id,$op=''){
        $modelName=$this->model;
        $model = $this->loadModel($id, $modelName);
        $w1='teacher_code='.Yii::app()->session['TCOD']. ' and f_year='.get_year();
        $tmp=$modelName::model()->findAll($w1.' and (ISNULL(F_ORDER) or f_order=0)');
        foreach ($tmp as $v){
            $v->f_order=$v->id;
            $v->save();
        }
        $tmp=$modelName::model()->findAll($w1.' order by f_order , id');
        $a=array();
        $r=0;
        $r0=0;
        foreach ($tmp as $v){
        ...
        if($r!=-1){
            ...
            $modelName::model()->updateAll(array('f_order'=>$odr),'id='.$a[$r]->id);
            $modelName::model()->updateAll(array('f_order'=>$odr0),'id='.$a[$r0]->id);
